...
# - each raw instrument maps to exactly one normalized instrument
# - each normalized instrument may map to many raw instruments
...
def _parse_yyyymmdd(s: str) -> Optional[dtmod.date]:
...
def _configure_plot_font(preferred_font: Optional[str]) -> Optional[str]:
    """
...
def _pick_best_norm_for_raw(raw: str) -> Optional[str]:
    counts = raw_norm_counts.get(raw)
    if not counts:
        return None
...
def load_outputs(glob_template: str="outputs/model_output/2026_02_14_t2_{batch}_deepseek-reasoner/*",
    ...
    for batch in range(3):
        ...
        for f in sorted(glob.glob(pattern)):
            ...
            date_str = _extract_date_str(f2)
            # Use utf-8-sig to tolerate UTF-8 BOM in upstream files.
            with open(f, "r", encoding="utf-8-sig") as fp:
                js = json.load(fp)["instruments"]
            for itm in js:
                raw_inst = _strip_bom(itm["instrument"])
                norm_inst = _strip_bom(itm["instrument_normalized"])
    ...
    for raw in raw_map_multi.keys():
        best_norm = _pick_best_norm_for_raw(raw)
